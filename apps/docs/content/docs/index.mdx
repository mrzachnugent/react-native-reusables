...
- **Animations**: Uses `react-native-reanimated` for smooth, native performance.
- **Icons**: Use a wrapper component like `<Icon as={LeftArrowIcon} />` to avoid wrapping every imported icon individually with `cssInterop`.
