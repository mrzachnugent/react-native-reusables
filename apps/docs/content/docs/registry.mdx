...
<Callout type="warn">
  **Note:** If you want to build universal components we recommend building your registry with React Native, even thought the registry works with any project type and any framework, and is
...
{/* TODO(gabimoncha): Update link to template or make repo public */}
...
This `registry.json` file must conform to the `shadcn` [registry schema specification](https://ui.shadcn.com/docs/registry/registry-json).
...
<Callout>
  **Note:** For more info about adding auth, guidelines, FAQs, more examples and json schemas you can check the following `shadcn` documentation pages:
  ...
  - [FAQs](https://ui.shadcn.com/docs/registry/faq)
